...
class TestStockStrategyEngine(unittest.TestCase):
    ...
    def test_three_prong_structure_generators(self):
        ...
        snap.update({"beta": 1.9, "adr_pct": 3.8, "rsi": 58.0, "macd_hook_ok": True})

        hr = stocks.structure_high_risk_stock_trade("SPRINT", "TECH SEMIS", snap, "EMA50", 1, portfolio_capital=100000.0)
        ...
        bal = stocks.structure_balanced_stock_trade("SWING", "TECH SOFTWARE", snap, "EMA50", 1, portfolio_capital=100000.0)
        self.assertEqual(bal["strategy_prong"], "BALANCED")
        self.assertEqual(bal["stagnation_limit_days"], 14)

        core = stocks.structure_core_stock_accumulation("CORE", "TECH CORE", snap, portfolio_capital=100000.0)
    ...
    def test_bifurcated_iv_scoring_behavior(self):
        ...
        from patterns import compute_options_alpha_score
        _, b_hr_high = compute_options_alpha_score(strategy_prong="HIGH_RISK", iv_rank=75.0, return_breakdown=True)
        _, b_hr_low = compute_options_alpha_score(strategy_prong="HIGH_RISK", iv_rank=20.0, return_breakdown=True)
        self.assertGreater(b_hr_high["iv_rank_efficiency"], b_hr_low["iv_rank_efficiency"])

        _, b_core_low = compute_options_alpha_score(strategy_prong="CORE", iv_rank=20.0, return_breakdown=True)
        _, b_core_high = compute_options_alpha_score(strategy_prong="CORE", iv_rank=75.0, return_breakdown=True)
